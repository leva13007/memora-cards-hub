---
id: TC-MC-0023
title: "UI-kit: TextLink – Mobile – Light Theme – Typography Tokens"
type: UI
priority: Medium
...
# TC-MC-0023 — UI: TextLink – Mobile – Light Theme – Typography & Link Tokens

## Objective

Validate that the `TextLink` UI component renders correctly on **Mobile** viewport in **Light theme**, using the expected typography design tokens (inheriting from `TextMedium`) and link-specific color tokens defined in ticket  
...
5. Configure the story or component props to use:
	...
	- final link should be as: [final link](https://leva13007.github.io/memora-cards-storybook/iframe.html?id=ui-atoms-textlink--default&viewMode=story&args=to:docs;content:The%20TextLink%20text%20content)
...
- In **Light theme**, when not hovered/pressed:
	- `color` = `#1a73e8` equals the resolved value of **`--color-link`**
- In **Light theme**, on hover/press:
...
- Screenshot of `TextLink` rendering on Mobile + Light theme (showing default state)

![alt text](../../assets/image-23.png)

- Screenshot of `TextLink` rendering on Mobile + Light theme (showing pressed/hover state)

![alt text](../../assets/image-23-h.png)
...
- Test very long `content` strings to verify wrapping and hover/tap behavior across multiple lines at mobile width.
...
- This test case covers only **Mobile (<768px) + Light theme** for the `TextLink` component.
- Separate test cases should cover Dark theme and Desktop/Tablet variants.
- Typography expectations should match `TextMedium` Mobile + Light behavior, except for using link-specific color tokens instead of `--color-text`.
